...
@property (weak, nonatomic) IBOutlet UIView *anchorView;//背景
@property (weak, nonatomic) IBOutlet UIImageView *headImageView;//主播头像
@property (weak, nonatomic) IBOutlet UILabel *nameLabel;//主播名字
@property (weak, nonatomic) IBOutlet UILabel *peopleLabel;//观看人数
@property (weak, nonatomic) IBOutlet UIButton *careBtn;//关注主播
@property (weak, nonatomic) IBOutlet UIScrollView *peoplesScrollView;//关注打赏人数
@property (weak, nonatomic) IBOutlet UIButton *giftView;//礼物数量
...
- (NSArray *)chaoYangUsers
{
    if (!_chaoYangUsers) {
        ...
        _chaoYangUsers = [ZlyUser mj_objectArrayWithKeyValuesArray:array];
    }
...
- (void)setupChangeyang
{
    ...
    for (int i = 0; i<self.chaoYangUsers.count; i++) {
        ...
        ZlyUser *user = self.chaoYangUsers[i];
...
- (void)clickChangYang:(UITapGestureRecognizer *)tapGes
{
    if (tapGes.view == self.headImageView) {
        ZlyUser *user = [[ZlyUser alloc] init];
